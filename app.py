...
# --- 5. PAGE: YEAR VIEW ---
    ...
    with st.sidebar:
        ...
        st.header(f"⚙️ {selected_year} Settings")
        t4_gross_income = st.number_input("T4 Gross Income", value=float(year_data.get("t4_gross_income", 0)), step=5000.0)
        base_salary = st.number_input("Annual Base Salary", value=float(year_data.get("base_salary", 0)), step=5000.0)
        
        st.header("💰 RRSP Setup")
        ...
        st.header("📅 Bulk Deposits")
        rrsp_lump_sum = st.number_input("RRSP Lump Sum", value=float(year_data.get("rrsp_lump_sum", 0)))
        tfsa_lump_sum = st.number_input("TFSA Lump Sum", value=float(year_data.get("tfsa_lump_sum", 0)))
